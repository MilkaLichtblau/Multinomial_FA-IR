...
\begin{figure}[t!]
	...
	\caption{Experiments on data generated by a simulation, showing the need for multiple tests correction.
		%
		The data has two protected groups, rankings of lengths $k$ are created by the stochastical process of a multinomial distribution (``rolling a 3-sided dice'') with $p_G = (0.33, 0.33)$.
		%
...
%
At each iteration as it requires to compute a new mTree which is expensive on its own.
%
...
%
Let furthermore $MT$ be a $mTree$, with node $MT_{G,i}=[m_1(i),\ldots,m_{|G|}(i)]$ giving the number of protected candidates of group $1,\ldots,|G|$ required up to position $i$.
%
...
%
Now lets assume that at level $9$, the given ranking would pass the test for node $(9, [2,1])$, which is not a successor of $(9, [1,1])$.
%
...
iff there exists a \removed{valid} path from the root of $MT$ to a leaf of $MT$ \changed{where $\tau$ passes the test at each node on that path}.
\end{theorem}
%
\meike{This proof is to be reviewed}
\begin{proof}
	\label{proof:lazy-mTree-test}
\changed{\paragraph{"$\Leftarrow$"}:
If there exists a path for which $\tau$ passes the test at each node, the exists a node on each level of $MT$ where $\tau$ passes the test because every path has a node on every level.
}
\removed{It is clear that at least one successful test per level is necessary for the path to exist.
	%
...
\changed{\paragraph{"$\Rightarrow$"}:}
	...
	%
	\removed{Without loss of generality} Let $\sum_{g=1}^{|G|} |m_{g}(h) - \tau_{g}(h)| = 0$, meaning that the ranking includes the exact amount of required protected candidates at level $h$ and not more. \changed{Note that this assumption reduces the number of nodes at which $\tau$ passes the test at level $h$ to a minimum. If we would assume that $\tau$ includes more protected candidates than required, we would only increase the number of nodes at level $h$ for which $\tau$ would pass the test. Thus, we can proof this direction with the above assumption without loss of generality.}
	%
