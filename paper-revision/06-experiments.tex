...
\spara{Baseline 1: Group-unaware or ``color-blind'' ranking.} This method merely order the candidates by decreasing scores/qualifications (see Section~\ref{concept:color-blind-ranking}) \textcolor{blue}{and therefore maximizes selection and ordering utility.
	%
...
\textcolor{blue}{\spara{Baseline 3: Rolling a $|G|$-sided dice at each position.} We adopt the statistical binomial process by~\cite{yang2016measuring} to a setting in which more than one protected group is present. 
	%
...
\textcolor{blue}{\spara{Kendall's Tau (Performance Measure). } We report the similarity of the fair candidate orderings w.r.t. the colorblind ranking in terms of the Kendall Tau correlation coefficient. 
	%
...
\begin{table}[t!]
	...
	\centering\begin{tabular}{llcccc}\toprule
   					&			& Exposure gain 	& Exposure gain		&   total   	& Kendall\\
...
When considering the dice rolling experiments, we see our expectations of non-consistent behavior confirmed. 
%
