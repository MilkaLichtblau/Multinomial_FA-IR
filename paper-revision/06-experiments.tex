...
\textcolor{blue}{\spara{Baseline 3: Rolling a $|G|$-sided dice at each position.} We adopt the statistical binomial process by~\cite{yang2016measuring} to a setting in which more than one protected group is present. 
	...
	%
	Starting with the first position, we roll the dice, put the best available candidate from the group whose side is showing and repeat the process for each of the top-$k$.
	%
	We note that this procedure, if repeated many times, will approximate the results of our methods. 
	%
