...
\spara{Baseline 2: \citet{zehlike2020matching}.} This is a post-processing method named Continuous Fairness Algorithm (CFA$\theta$)\changed{, and as explained in Section~\ref{subsec:fair-ranking}, it is the only utility-based method in the literature that does not assume that utilities can be compared across groups}.
%
...
\changed{\spara{Baseline 3: Rolling a $|G|$-sided dice at each position.} We adopt the statistical binomial process by~\cite{yang2016measuring} to a setting in which more than one protected group is present. 
	%
...
\changed{\spara{Kendall's Tau (Performance Measure). } We report the similarity of the fair candidate orderings w.r.t. the colorblind ranking in terms of the Kendall Tau correlation coefficient. 
	%
...
\begin{table}[t!]
	...
	\centering\begin{tabular}{llcccc}\toprule
   					&			& Exposure gain 	& Exposure gain		&   total   	& \changed{Kendall}\\
...
%
\changed{However, we see that \algoFAIR outperforms CFA$\theta$ significantly in terms of Kendall's tau, which means that the fair rankings constructed by CFA$\theta$ have a lot more ordering inversions w.r.t. the colorblind ranking than those constructed by \algoFAIR. }
...
%
\changed{This problem also explains the large differences between the methods in Kendall's Tau.}
...
\changed{When considering the dice rolling experiments, we see our expectations of non-consistent behavior confirmed. 
	%
...
%
An important advantage of \algoFAIR is, that it allows to create rankings for user-defined values of $p$ and in particular for values beyond statistical parity, something that cannot be done directly with CFA$\theta$ (\citet{zehlike2020matching} allows at maximum statistical parity in the result ranking when setting $\theta=1$, i.e. to the maximum value).
