...
%
On line \ref{alg:fair:mtree}, the algorithm creates a ranked group fairness tree (mTree) similar to the one shown on Figure~\ref{fig:mtree-symmetric-adjusted}, indicating the minimum number of protected candidates needed at every position. % group, given $p_G$, $k$ and $\alphaadj$.
%
...
\changed{
\subsection{Scalability and MTree Recycling}
...
Once computed, a mTree can be used to test and/or create rankings according to our definition of a fair ranking. The mTree itself is independent of the data tested or ranked and thus is reusable for all rankings of length $m\leq k$ with required proportions $p_G$ and significance $\alpha$. Since testing and ranking with mTrees is efficient, but mTree creation is not, it is important to store already computed mTrees and never compute a mTree twice. Our implementation \footnote{\url{https://github.com/MilkaLichtblau/Multinomial_FA-IR}} provides the infrastructure for loading and storing mTrees for all steps of our algorithm. We also provide the set of pre-computed mTrees of our experiments.
\subsubsection{Recycling mTrees}
As mentioned above, an mTree of depth $k$ can be used to create and test rankings of every length up to and including $k$. Thus for given $\alpha$, $p_G$ only the largest mTree has to be stored. Furthermore, all mTrees calculated for the binary search of the adjusted mTree can be reused too if only the parameter $\alpha$ changes. More specifaically, algorithm \ref{alg:regression_search} computes mTrees which can be stored permanently. Then if we want to compute an mTree for the same $p_G$ but with a different $\alpha$, many of the binary search steps (computed mTrees) can be reused. In summary, the first calculation of a large mTree for many protected groups is expensive, but has to be done only once. After that, adjustments of $k$ and $\alpha$ are fast through recycling of already computed mTrees and MCDFs (see section \ref{subsec:further-optimizations}). The only parameter in which mTrees can not be recycled easily, is if $p_G$ changes. However, in many cases $p_G$ will most certainly be determined by policy, company guidelines or demographic data, which should change not often.}
