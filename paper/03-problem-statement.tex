...
We consider a set of candidates $[n] = \{ 1, 2, \dots, n \}$  out of which $k$ candidates will be selected. We are also given $q_c$ for each $c \in [n]$, denoting the ``utility'' of candidate $c$: this can be interpreted as an overall summary of the fitness of candidate $c$ for the specific job, task, or search query, and it could be obtained by the combination of several different attributes.
...
\end{definition}
With the multinomial CDF the ranked group fairness criterion is formulated as a statistical significance test, and we include a significance parameter ($\alpha \in [0,1]$) corresponding to the probability of rejecting a fair ranking (i.e., a Type I error).
...
%
Other problem definitions with respect to the general criteria described in Section~\ref{concept:criteria} are possible.
%
