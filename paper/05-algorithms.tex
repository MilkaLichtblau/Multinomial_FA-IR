...
\note[ChaTo]{I suggest to move this entire subsection (\S\ref{subsec:algorithm-optimization}) to the previous section (\S\ref{sec:alpha-adjustment-multi}). That way here we would assume the mTrees were already created in the best/cheapest possible way.}
...
\begin{table}[t]
	...
	\scalebox{0.75}{
		\begin{tabular}{lll}
			...
			\algoImcdf & $\mathcal{O}(|G|) \cdot \mathcal{O}(MCDF(k,p,\alpha ))$ & $\mathcal{O}(|G|^2)$ \\
			\rowcolor[HTML]{C0C0C0}
			\algoComputeMTree & $\mathcal{O}(|G|^{k}) \cdot \mathcal{O}(\algoImcdf)$ & $\mathcal{O}(|G|^{k})$\\
			\algoMultBinary & $\mathcal{O}(\log{}\frac{\alpha}{\epsilon}) \cdot (\mathcal{O}(k^2) + \mathcal{O}(\algoComputeMTree))$  & $\mathcal{O}(|G|^k)$\\
			\rowcolor[HTML]{C0C0C0}
			\algoReg & $\mathcal{O}(\log{}\frac{\alpha}{\epsilon}) \cdot (\mathcal{O}(k^2) + \mathcal{O}(\algoComputeMTree))$ & $\mathcal{O}(|G|^k)$\\
			\algoFAIR & $\mathcal{O}(n \log{} n) + \mathcal{O} (\algoMultBinary) + \mathcal{O}(k)$ & $\mathcal{O}(|G|^k + n + k)$ \\
			\bottomrule
		\end{tabular}
	}
\end{table}

\todo[Meike]{@Chato: Maybe we put the comments on space and time complexity in the appendix?}
\note[ChaTo]{I'm thinking they fit well in the main text (they don't need to go to an appendix), but perhaps would put the analysis on the complexity of mTree generation and model adjustment in the previous section (\S\ref{subsec:multinomial-adjustment}) and the complexity of FA*IR when the mTable/mTree is created in this section (\S\ref{sec:experiments})}
