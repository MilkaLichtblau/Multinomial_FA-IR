...
\note[ChaTo]{I'm thinking they fit well in the main text (they don't need to go to an appendix), but perhaps would put the analysis on the complexity of mTree generation and model adjustment in the previous section (\S\ref{sec:multinomial-adjustment}) and the complexity of FA*IR when the mTable/mTree is created in this section (\S\ref{sec:experiments})}
