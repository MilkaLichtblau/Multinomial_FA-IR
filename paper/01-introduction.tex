...
% RIC: "a minimum proportion"? What is G? Groups? Can we just say "a minimum proportion"? G is not really needed and can be confusing
%
...
% RIC: What it means that a probability belongs to a proportion? Is p_G a set? (that is not clear before)
%
...
\begin{table}[t]
  ...
  \note[ChaTo]{Table in its current version is inconsistent: says non-white and black.}
...
\end{compactenum}
% RIC: I would not claim that a parts of an algorithm that is also a contribution is a different contribution, less if no other algorithm we use them
...
% RIC: A correction or an improvement? Would be better to say the second
%
