...
\begin{table}[t]
	...
	\resizebox{1.02\columnwidth}{!}{%
		\centering\begin{tabular}{llcccccc}\toprule
			...
			\midrule
			D1, k=1500 & \algoFAIR $p_{\text{eq}}$ & 0.27 & $1.915\cdot10^{-7}$ & 0.0066 & 15 & 0.0000 \\
			COMPAS, & \algoFAIR $p_{\text{stat}}$ & 19.16 & 0.0039 & 0.0687 & 387 & 0.0621 \\
			race (1 prot.) & \citeauthor{zehlike2020matching} &  &  &  &  &  \\ 
			\midrule 
			
			D2, k=500 & \algoFAIR $p_{\text{eq}}$ & $[-13.77, -7.10, 20.87]$ & 0.0580 & $[0.23, 0,23, 0.00]$ & $[181, 127, 0]$ & $[0.23, 0.23, 0.00]$ \\
			COMPAS, & \algoFAIR $p_{\text{stat}}$ & $[-20.86, 6.75,14.11]$ & 0.0354 & $[0.21, 0.21, 0.00]$ & $[266, 3, 0]$ & $[0.21, 0.21, 0.00]$ \\
			age (2 prot.) & \citeauthor{zehlike2020matching} & $[]$ &  & $[]$ & $[]$ & $[]$ \\ %\cline{2-7} 
			\midrule
...
%
\meike{Revise this: Second it is important to note that the baseline by~\citet{zehlike2020matching} does not guarantee in-group monotonicity, which we observed in our experiments.
%
For example, in experiment D2 we observe a maximum rank drop for the non-protected group of 159. 
%
When investigating in the 
%
Second, \algoFAIR achieves higher or equal selection utility than the baseline~\cite{zehlike2020matching} in all but one of the experimental conditions (D7).
%
