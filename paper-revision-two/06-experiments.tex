...
%
\changed{However, D1 -- COMPAS reveals one strength of the binomial \algoFAIR implementation and the guarantees we proved for the binomial case. We would expect no difference for one protected group between multinomial \algoFAIR and binomial \algoFAIR. However, as desribed in section \ref{sec:model-adjustment}, the significance adjustment of the multinomial method has variance in its' precision. This has no impact on the fair ranking for small $p$ and smaller $k$. In this case of $k=1500$ even a small difference can lead to slightly different results. Thus we recommend to use the binomial version of \algoFAIR if only one protected group exists, as it guarantees the best possible significance adjustment \cite{zehlike2020note}.}
%
